document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBtn.addEventListener('click', function() {
      ...
      for (var i = 0; i < days.length; i++) {
        ...
        // Get meal options
        fetchMealOptions(day, tabContent);
    ...
    function fetchMealOptions(day, tabContent) {
        // Spoonacular API request
        var spoonacularAPIKey = 'YOUR_SPOONACULAR_API_KEY';
        var spoonacularEndpoint = 'https://api.spoonacular.com/recipes/random';
        var spoonacularParams = {
          apiKey: spoonacularAPIKey,
          number: 1
        };
      
        fetch(spoonacularEndpoint + '?' + new URLSearchParams(spoonacularParams))
          .then(response => response.json())
          .then(data => {
            var meal = data.recipes[0].title;
      
            var mealElement = document.createElement('p');
            mealElement.textContent = 'Meal: ' + meal;
      
            tabContent.appendChild(mealElement);
          })
